...
|------|-----------|-------------|
| `code_mode` | `code`, `settle_frames?`, `real_frames?` | Execute Python live on TD's main thread with a curated `tk.` toolkit (make/wire/layout/setp/find/externalize/settle/errors/checkpoint/report/after_frames) over native `td`. After your code runs, TD force-cooks the touched ops and the response carries CONSOLIDATED diagnostics -- op errors + warnings + GLSL shader-compile logs -- so you never poll. Fresh globals each call. `tk.make` auto-externalizes new COMPs per the `Autoexternalize` preference (same as `create_op`). **Read-after-write:** the force-cook cannot advance real frames, so a read that needs a callback to fire / a Movie File In reload to land / a feedback loop to evolve is stale in the same call -- pass `real_frames=N` and put the READ in a `tk.after_frames(fn)` callback; the call defers across N real frames then runs it. |
