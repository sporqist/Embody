...
[![Version](https://img.shields.io/badge/version-6.0.141+cm.5-6ee668?style=flat-square&labelColor=181e1e)](https://github.com/sporqist/Embody/releases/latest)
...
**Versioning.** This fork's version is a frozen upstream anchor plus its own
build counter — `6.0.141+cm.4`. The `6.0.141` part is the last version
[upstream](https://github.com/dylanroscover/Embody) published before this fork
diverged, and states which upstream Embody/Envoy a build is compatible with; it
never moves. Only `cm.N` increments. Builds originally numbered 6.0.142–6.0.144
were this fork's builds mistakenly taken from upstream's line, and are
renumbered `+cm.1`–`+cm.3` below.

**Recent releases:**

- **6.0.141+cm.4**: Fixes two silent data-corruption bugs found by an agent field report — the STDIO transport was CP1252-decoding all non-ASCII input (an em dash arrived as three characters, corrupting parameter labels permanently on write), and TDN exports silently dropped the code of any DAT that had no externalized file (one project's `.tdn` held 185 operators and zero lines of code). Authored DAT content is now never dropped, new DATs under a non-embedding `.tdn` are auto-externalized to their own `.py`, and the versioning scheme above was corrected.
- **6.0.141+cm.1** *(published as 6.0.142)*: The Envoy **code-mode surface** — three LLM-ergonomic tools landed **additively** alongside the existing 53. **`code_mode`** runs Python live on TD's main thread with a curated `tk.` toolkit (`make`/`wire`/`layout`/`setp`/`find`/`settle`/`report`) and returns consolidated diagnostics after an auto-settle (op errors + warnings + GLSL compile logs the model would otherwise miss). **`describe`** is read-as-text: `contract` (the `tk` surface), summary-first `node` (a one-line op summary; `full=True` fans out params with sequences collapsed), `network` (topology + optional sparse TDN dump), and `docs` (live introspection **fused** with offline-wiki prose — "never guess parameters"). **`view`** is read-as-render: TOP → inline image, CHOP/DAT → reduced stats/rows, plus relational (op-vs-op) and temporal diffs. Menu params now surface friendly labels + options inline (never raw indexes). An opt-in **`SetToolSurface('codemode')`** hides the 53 verbs behind the 3. Nothing removed, all prior tests green. **2,221 tests passing** (102 suites; `test_codemode`/`test_describe`/`test_view`/`test_tool_surface`).
