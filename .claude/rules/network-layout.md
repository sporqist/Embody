...
**Tool surface note (code-mode default).** In the default 3-tool code-mode surface, `tk` does most manual positioning for you: `tk.make(optype, ...)` auto-positions the new op clear of its siblings AND hugs its docked companions (same as `create_op`); `tk.wire(*ops)` only CONNECTS ops (it does not move them) -- `tk.layout(*ops)` (or `tk.wire(*ops, layout=True)`) is what arranges a chain left-to-right. So the create/connect/position verbs below map to code-mode as: `create_op` -> `tk.make`, `connect_ops` -> `tk.wire`, `set_parameter` -> `tk.setp` / native `op.par.X`, `set_op_position` / `execute_python` / `batch_operations` -> `code_mode` (write the loop, set `nodeX`/`nodeY` natively or via `tk.layout`), `get_network_layout` -> read `op.nodeX/nodeY/nodeWidth/nodeHeight` + `op.docked` (the dockedTo check) natively in `code_mode` (`describe` does NOT return geometry); `get_annotations` -> `code_mode` with `parent.findChildren(type=annotateCOMP, includeUtility=True)` (annotations are utility ops, hidden from `describe(network)`). The verb tools remain valid under `SetToolSurface('full')`. Either way, the **Verify step and every geometry rule below STILL apply** -- a raw `code_mode` / native `.create()` / `.copy()` build gets no auto-layout beyond what `tk` gives you, so you own placement and the Verify gate exactly as with `execute_python`.
