...
!!! note "Versioning"
    This fork's version is a **frozen upstream anchor plus its own build counter**: `6.0.141+cm.N`. The `6.0.141` part is the last version [upstream](https://github.com/dylanroscover/Embody) published before this fork diverged (commit `2c980a8`), and states which upstream Embody/Envoy a build is compatible with — it never moves. Only `cm.N` increments, once per save. Entries below `6.0.141` are upstream's own history and keep their original numbers.

## 6.0.141+cm.5

Acts on the rest of the agent field report and selectively hand-ports fixes from upstream (surveyed `2c980a8`..`v6.0.259`; a literal rebase is impractical -- the shared extensions took +900/-200 lines in `EmbodyExt.py` alone). Full suite: 2268 passed, 0 failed, 7 skipped.

- **`code_mode` read-after-write (`real_frames` + `tk.after_frames`)**: the field report's single most-requested fix (raised four times). `code_mode`'s auto-settle is a synchronous force-cook -- it cannot advance real frames, so a read in the same call after a write that needs real frames (a callback firing, a Movie File In reload landing, a feedback loop evolving) gets stale, always-plausible data. Pass `real_frames=N`, do the write in the body, and put the read in a `tk.after_frames(fn)` callback: the call defers across N real frames, then runs the callback and returns its `tk.report()`. Mirrors the proven `run_tests` deferral (worker Event + a `run(delayFrames=1)` chain); `real_frames=0` (default) leaves the synchronous path byte-for-byte unchanged. Verified live: an `onFrameStart` counter advanced by exactly `real_frames`.
- **Sparse input connections survive TDN round-trip** (ported from upstream v6.0.241): the exporter enumerated `OP.inputs`, which is COMPACTED, so a wire on input 1 with input 0 empty collapsed onto input 0 on reimport (a crossTOP's right input silently moved to the left). Now enumerates positional `inputConnectors`. Reproduced the bug on TD 2025.33070 before fixing.
- **Runtime storage keys no longer churn into `.tdn`** (ported from upstream v6.0.160): the periodic counters `_watchdog_gen` / `_clip_watch_gen` / `_shortcut_rec_gen` ticked every few seconds, so every save baked a fresh value into `Embody.tdn`; `claudius_running` and the startup/restore flags leaked the same way. Added to `SKIP_STORAGE_KEYS`.
- **UTF-8 subprocess reads** (ported from upstream v6.0.252): six `subprocess.run` calls that decode output used the locale codepage (US-ASCII on macOS GUI TD, CP1252 on Windows), so one non-ASCII byte from uv/pip/git could crash the venv build. Now forced to `utf-8`/`errors='replace'`.
- **Clipboardautopaste release-leak fixed** (ported from upstream v6.0.252): the release `.tox` is exported from the live dev COMP, so `Clipboardautopaste=False` on the dev machine shipped auto-paste OFF to every user. `ExportPortableTox` now resets ship-at-default preference params to their default for the exported artifact and restores the live value afterward.
- **Inert self-watching Parameter Execute DAT is now flagged**: a Parameter Execute DAT whose watch target is the COMP it lives in is inert (TD's recursion guard) -- no error, nothing fires. `code_mode`'s diagnostics now warn on it. And `describe(node, full)` spells out that sequence pages are **0-indexed** (`op.seq.<name>[i] for i in 0..N-1`), so an agent reads the origin instead of guessing it.
- **Field report 2.3 (TDN blank rows) corrected, not "fixed"**: it does not reproduce on TD 2025.33070 through any Embody path (`oper.save()` writes clean CRLF; a `\r\r\n` file loads as clean rows; a `\r`-containing cell round-trips losslessly). No speculative "strip empty rows" guard ships -- it would corrupt a table with a legitimately empty row.
- **Two flaky clipboard tests de-flaked**: `test_clipboard_paste` and `test_clipboard_watch` wrote the real OS clipboard and read it back, so a second process could clobber it mid-test. Added a `clipboard_str` override (production reads `ui.clipboard` as before) and routed the tests through it.

## 6.0.141+cm.4

Fixes two silent data-corruption bugs found by an agent field report after an extended build session (see `MCP-SERVER-ASSESSMENT.md`), and corrects this fork's versioning.

- **Versioning corrected — the fork no longer claims upstream version numbers.** `onProjectPreSave` incremented the last dotted segment of `par.Version` on *every* save, so each save walked the **upstream** triple forward. Three builds (`6.0.142`, `6.0.143`, `6.0.144`) were this fork's, numbered from upstream's line, and a release was published advertising compatibility with an upstream `6.0.144` that does not exist. The version is now `<frozen upstream anchor>+cm.<counter>`: the save hook parses that shape explicitly and bumps **only** the counter, refusing to guess at anything else rather than silently mis-numbering a build. The prior builds are renumbered `+cm.1`–`+cm.3`, and the mis-numbered `v6.0.143` / `v6.0.144-codemode` releases were replaced. Guarded by `test_version_never_claims_an_upstream_number` and `test_hook_bumps_only_the_build_counter`; the README badge rewriter and its sync test were widened to match the suffix (their old `[0-9.]*` pattern stops at the `+`, so the bumper would have silently no-oped after the first transition — the exact failure mode that test suite exists to catch).

- **STDIO transport is pinned to UTF-8 (silent mojibake fix)**: MCP clients send UTF-8 JSON-RPC, but Python binds `sys.stdin`/`stdout` to the *locale* codepage -- CP1252 on a Western Windows install. Every non-ASCII character sent through the bridge was silently CP1252-decoded on arrival: an em dash (U+2014, bytes `E2 80 94`) became the three characters `E2/20AC/201D`. Nothing raised, the value looked correct in the JSON response, and because it corrupted on *write* the damage was saved permanently into `.toe` / `.tdn` files -- it had already reached parameter labels and annotation titles in a real project. `envoy-bridge.py` now calls `pin_stdio_utf8()` from `main()` (deliberately not at import: the unit tests exec the bridge inside TouchDesigner, and an import-time pin would mutate TD's own streams). The outbound half was already safe by accident, since `json.dumps` defaults to `ensure_ascii=True`.
- **DAT code is never silently dropped from a TDN export**: `include_dat_content=False` now means "skip content that is ALREADY saved elsewhere", never "throw code away". A DAT backed by its own externalized file is still omitted (no duplication); a DAT with **no** file is always embedded. Previously a TDN-strategy COMP with embedding off (the default) exported complete structure and *zero* lines of code -- one project lost every shader, callback and script to a crash exactly this way, while the UI showed a healthy green "Saved". Runtime-generated DATs (rolling `fifoDAT` buffers, derived listings, live streams) and DATs fed by an input are excluded from the safety net, reusing the same canonical `_TD_MANAGED_DAT_TYPES` set the save-time at-risk check uses so the two features cannot disagree.
- **New DATs under a non-embedding `.tdn` are auto-externalized**: `AutoExternalizeNewOp` previously skipped any op with an externalized ancestor, on the assumption that the ancestor's `.tdn`/`.tox` already captured it. That holds for a `.tox` (a full binary snapshot) but **not** for a `.tdn` with embedding off, which stores structure only. A DAT created there now gets its own `.py` -- which the `.tdn` references rather than embeds -- so authored code lands in a diffable file by default instead of a YAML blob. Tagging continues to ride along on the create the agent already performs, so it can never be forgotten.
- **Warning steers to the better remedy**: an export that would be the only copy of some DAT's content now recommends *externalizing* (`tk.externalize(op)` or the tagger) first, mentioning embedding only as the alternative.
- **Record corrected**: the field report's claim that DATs inside a TDN-strategy COMP cannot be externalized is false -- verified live, tagging one succeeds. The quoted refusal is a *type* check that fired on an auto-generated Info DAT; GLSL shader DATs are plain `text` DATs and were externalizable all along.
...
- **`code_mode` -- execute Python live with the `tk` toolkit + consolidated diagnostics**: write ordinary Python that runs on TD's main thread with full native `td` in scope, plus a curated `tk.` namespace that removes the common footguns: `tk.make` (create + auto-position + hug docked companions + set pars), `tk.wire` (variadic chain connect, `layout=True` arranges left-to-right), `tk.layout`, `tk.setp` (batch set, clear error on typo), `tk.find`, `tk.settle`, `tk.errors`, `tk.checkpoint`, `tk.report`. Fresh globals every call. After the code runs, TD force-cooks the touched ops and the response carries CONSOLIDATED diagnostics -- op errors + warnings + GLSL shader-compile logs scraped from the docked Info DAT (`op.errors()` misses those; verified on 2025.33070) -- so the model never has to poll. New suite `test_codemode` (20 tests).
- **`describe` -- read-as-text: contract / node / network / docs**: one tool, required `mode` + `target`, no smart dispatch. `contract` returns the `tk.*` surface (generated from the live toolkit, never drifts); `node` is summary-first (a synthesized one-line op summary + connections by default, `full=True` fans out custom + non-default pars with parameter SEQUENCES collapsed into blocks instead of `const0value`/`const1value`/...); `network` gives topology to depth N with an optional sparse TDN dump; `docs` FUSES live introspection (authoritative param names + defaults from a transient probe of the running build) with offline-wiki prose (Summary + Parameters). Menu params surface their friendly label + full option list inline (value stays the token, never a raw index). New suite `test_describe` (19 tests).
- **`view` -- read-as-render: TOP image + CHOP/DAT reduction + diff**: TOP -> inline PNG downscaled to a tunable cap (default 480 longest edge) with the black/empty-frame Quality verdict; CHOP -> per-channel stats + head/tail samples (never a blind dump); DAT -> header + head/tail rows. Two diff axes: relational (op-vs-op -- what a chain does to its data) and temporal (auto vs this client's last view; `pin=True` for a sticky baseline). New suite `test_view` (12 tests).
- **Hide the 53 behind the 3 (`SetToolSurface`)**: an opt-in server flag (`full` default, `codemode`) prunes the FastMCP tool manager to expose ONLY `code_mode`/`describe`/`view` (bridge meta-tools unaffected). Persisted in the .toe; the choice is stamped on the main thread at server launch and read worker-side (thread-boundary safe). New suite `test_tool_surface` (6 tests).
- **NOTICE**: root attribution crediting the reused Embody/Envoy infrastructure (transport, main-thread marshaling, `get_op_errors`, layout helpers, TDN) under MIT.
- Tests: 102 suites, ~2,228 tests (57 new across `test_codemode`/`test_describe`/`test_view`/`test_tool_surface`); full normal-tier run 2,221 passed / 0 failed / 7 platform-conditional skips.
