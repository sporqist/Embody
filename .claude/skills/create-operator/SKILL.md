...
**Codemode surface (default):** In the default 3-tool codemode surface you create operators with `tk.make(optype, name=None, parent=None, **pars)` inside `code_mode` -- it auto-positions the new op, hugs its docked companions, sets the given pars, and auto-externalizes new COMPs per the `Autoexternalize` pref (exactly what `create_op` does). The steps below still apply verbatim -- only the tool invocations change: wherever a step says "`create_op`", do `tk.make` in `code_mode`; wherever it says "`get_op_errors` after", read the CONSOLIDATED diagnostics `code_mode` returns after its auto-settle (or call `tk.errors(target)`). Other verb references map the same way: `execute_python` -> `code_mode` (use `tk.report(value)` to return a read-back value); `set_op_position` -> native `op.nodeX/nodeY = ...` (position is an op ATTRIBUTE, not a parameter -- `tk.setp` sets pars only); `connect_ops` -> `tk.wire(*ops)`; `query_network` -> `describe(mode="network")`; `get_network_layout` -> read `op.nodeX/nodeY/nodeWidth/nodeHeight` + `op.docked` natively in `code_mode`; `capture_top` -> `view(top)`; `externalize_op` -> `tk.externalize(op)`; `copy_op` -> native `.copy()` + `tk.layout(...)`. `create_op` and the other verb tools remain valid and are the full-mode path (restore them with `op.Embody.ext.Envoy.SetToolSurface('full')`).
